...
func (r *ApiKeyRepo) PrimaryForUser(ctx context.Context, userID string) (*entities.ApiKey, error) {
	var raw string
	err := r.s.DB.QueryRowContext(ctx, `SELECT payload FROM config_records WHERE entity='api_key' AND json_extract(payload,'$.owner_type')='user' AND json_extract(payload,'$.owner_user_id')=? ORDER BY (coalesce(json_extract(payload,'$.context_organization_id'),'')='') DESC,(substr(json_extract(payload,'$.created_at'),1,19) || '.' || CASE WHEN substr(json_extract(payload,'$.created_at'),20,1)='.' THEN substr(replace(substr(json_extract(payload,'$.created_at'),21),'Z','') || '000000000',1,9) ELSE '000000000' END),key LIMIT 1`, userID).Scan(&raw)
