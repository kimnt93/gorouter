...
func TestSwaggerDocumentsEveryJSONRoute(t *testing.T) {
	...
	want := map[string][]string{
		...
		"/admin/api-keys": {"get", "post"}, "/admin/api-keys/{id}": {"patch", "delete"}, "/admin/api-keys/{id}/rotate": {"post"}, "/admin/models": {"get"}, "/admin/models/{name}": {"put", "delete"}, "/admin/prices": {"get"}, "/admin/prices/{model}": {"put", "delete"}, "/admin/pricing/catalog": {"get"}, "/admin/pricing/estimate": {"get"}, "/admin/usage/summary": {"get"}, "/admin/usage/recent": {"get"}, "/admin/cache/stats": {"get"}, "/admin/cache/flush": {"post"},
	}
