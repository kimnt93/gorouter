...
test('loads cached models and refreshes explicitly without losing the existing list on failure', async () => {
  api.getProviders.mockResolvedValue({ data: [{ id: 'devin-cli', name: 'Devin', auth: 'api_key', description: '', protocol: 'devin-cli', quota_supported: false }] })
  api.getCredentials.mockResolvedValue([{ id: 'new', name: 'Devin account', provider: 'devin-cli', kind: 'api_key', status: 'active', created_at: '', base_url: '' }])
  api.discoverModels.mockResolvedValueOnce({ data: [{ id: 'future-model', public_id: 'dv/future-model' }] }).mockRejectedValueOnce(new Error('Upstream unavailable')).mockResolvedValueOnce({ data: [{ id: 'latest-model', public_id: 'dv/latest-model' }] })
  render(<ProvidersPage />)
  fireEvent.click(await screen.findByRole('button', { name: 'Models' }))
  expect(await screen.findByText('dv/future-model')).toBeInTheDocument()
  expect(api.discoverModels).toHaveBeenCalledWith('new')
  fireEvent.click(screen.getByRole('button', { name: 'Refresh from provider' }))
  expect(await screen.findByText('Upstream unavailable')).toBeInTheDocument()
  expect(api.discoverModels).toHaveBeenCalledWith('new', true)
  expect(screen.getByText('dv/future-model')).toBeInTheDocument()
  fireEvent.click(screen.getByRole('button', { name: 'Refresh from provider' }))
  expect(await screen.findByText('dv/latest-model')).toBeInTheDocument()
  expect(screen.getByText('Model catalog refreshed from provider')).toBeInTheDocument()
})
