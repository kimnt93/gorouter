...
test('reads cached quota and contacts the provider only after Reload', async () => {
  ...
  api.getCredentials.mockResolvedValue([{ id: 'cred-1', name: 'Work Codex', provider: 'codex', kind: 'oauth', base_url: 'https://chatgpt.com/backend-api', status: 'active', owner_tenant_id: null, created_at: '' }])
  api.getCredentialQuota.mockResolvedValue({ credential_id: 'cred-1', provider: 'codex', account: 'pe****@example.test', available: true, windows: [], message: 'Click reload to fetch quota' })
