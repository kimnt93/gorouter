...
export function CredentialsPage() {
  ...
  const update = async (item: Credential) => { try { await updateCredential(item.id, { name: item.name, base_url: item.base_url, status: item.status === 'active' ? 'disabled' : 'active', api_key: '', oauth_access: '', oauth_refresh: '', owner_tenant_id: item.owner_tenant_id }); await load() } catch (reason) { setError((reason as Error).message) } }
