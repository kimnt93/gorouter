...
import { createUser, deleteUser, getUsers, setUserStatus } from '../api/client'
...
export function UsersPage() {
  ...
  const remove = async (user: User) => { if (!window.confirm(`Delete ${user.username}? This permanently deletes their API keys, provider connections, memberships, and access.`)) return; try { await deleteUser(user.id); await load() } catch (reason) { setError((reason as Error).message) } }
...
function CreateUserModal({ onClose, onCreated }: { onClose: () => void; onCreated: (secret: string) => void }) {
  ...
  const submit = async () => { setBusy(true); try { const response = await createUser(username); onCreated(response.initial_key?.plaintext ?? '') } catch (reason) { setError((reason as Error).message) } finally { setBusy(false) } }
  return <Modal title="Create user" onClose={onClose}><ErrorBanner message={error} /><Field label="Email username"><input type="email" value={username} onChange={(event) => setUsername(event.target.value)} placeholder="person@example.com" /></Field><p className="safe-note"><strong>Automatic access</strong><span>The generated key receives management scopes and the virtual <code>auto</code> model. Copy it once after creation.</span></p><div className="dialog-actions"><button className="button" disabled={busy || !username.trim()} onClick={() => void submit()}>Create user and key</button></div></Modal>
}
