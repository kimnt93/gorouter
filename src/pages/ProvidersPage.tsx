...
export function ProvidersPage() {
  ...
  return <>
    ...
    {credentials.some((credential) => credential.provider === 'devin' || credential.provider === 'devin-desktop') && <section className="panel"><h2>Retired Devin connections</h2><p>The former Cloud and Desktop integrations are no longer routed. Connect through Devin below and import its live model catalog. Existing records remain in <a href={viewOrganizationID ? `/dashboard/credentials?organization_id=${encodeURIComponent(viewOrganizationID)}` : '/dashboard/credentials'}>Connection inventory</a> for removal; credentials are not automatically converted.</p></section>}
    ...
    <ProviderSection title="API-key providers" detail="API keys, imported subscription keys, and custom endpoints." providers={grouped.api} credentials={credentials} onConnect={setConnect} onModels={setModelsFor} onChat={setChatFor} onChatAll={(provider, accounts) => setChatAllFor({ provider, accounts })} onTestAll={(provider, accounts) => setTestAllFor({ provider, accounts })} onRefresh={load} />
...
function ProviderCard({ provider, accounts, onConnect, onModels, onChat, onChatAll, onTestAll, onRefresh }: { provider: ProviderDefinition; accounts: Credential[]; onConnect: (provider: ProviderDefinition) => void; onModels: (credential: Credential) => void; onChat: (credential: Credential) => void; onChatAll: (provider: ProviderDefinition, accounts: Credential[]) => void; onTestAll: (provider: ProviderDefinition, accounts: Credential[]) => void; onRefresh: () => Promise<void> }) {
  ...
  return <article className={`provider-card-react ${accounts.length ? 'has-accounts' : ''}`}><div className="provider-card-head"><span className="provider-monogram">{provider.id === 'devin-cli' ? <img src="/app-assets/assets/windsurf.svg" alt="" aria-hidden="true" /> : provider.name.slice(0, 2)}</span><div><h3><TruncatedText>{provider.name}</TruncatedText></h3><p title={provider.description}>{provider.description}</p></div><Badge tone={accounts.length ? 'good' : ''}>{accounts.length ? `${accounts.length} connected` : provider.auth}</Badge></div>
...
function ModelsModal({ credential, canImport, onClose }: { credential: Credential; canImport: boolean; onClose: () => void }) {
  ...
  const reload = async () => { setLoading(true); setError(''); setMessage(''); try { const response = await discoverModels(credential.id, true); setModels(response.data); setSelected((previous) => previous.filter((id) => response.data.some((model) => model.id === id))); setMessage('Model catalog refreshed from provider') } catch (reason) { setError((reason as Error).message) } finally { setLoading(false) } }
