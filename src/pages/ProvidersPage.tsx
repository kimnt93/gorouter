...
function ProviderCard({ provider, accounts, onConnect, onModels, onChat, onChatAll, onTestAll, onRefresh }: { provider: ProviderDefinition; accounts: Credential[]; onConnect: (provider: ProviderDefinition) => void; onModels: (credential: Credential) => void; onChat: (credential: Credential) => void; onChatAll: (provider: ProviderDefinition, accounts: Credential[]) => void; onTestAll: (provider: ProviderDefinition, accounts: Credential[]) => void; onRefresh: () => Promise<void> }) {
  ...
  return <article className={`provider-card-react ${accounts.length ? 'has-accounts' : ''}`}><div className="provider-card-head"><span className="provider-monogram">{(provider.id === 'devin' || provider.id === 'devin-desktop' || provider.id === 'devin-cli') ? <img src="/app-assets/assets/windsurf.svg" alt="" aria-hidden="true" /> : provider.name.slice(0, 2)}</span><div><h3><TruncatedText>{provider.name}</TruncatedText></h3><p title={provider.description}>{provider.description}</p></div><Badge tone={accounts.length ? 'good' : ''}>{accounts.length ? `${accounts.length} connected` : provider.auth}</Badge></div>
...
function ConnectProviderModal({ provider, onClose, onConnected }: { provider: ProviderDefinition; onClose: () => void; onConnected: () => void }) {
  ...
  return <Modal title={`Connect ${provider.name}`} onClose={onClose}><ErrorBanner message={error} /><SuccessBanner message={status} /><div className="form-grid"><Field label="Connection name"><input value={name} onChange={(event) => setName(event.target.value)} /></Field></div>
    {provider.auth === 'api_key' ? <>{provider.id === 'devin' && <p className="safe-note">Devin Cloud accepts current cog_ personal access tokens and service-user API keys. It validates through the documented v3 API. Devin Cloud is an asynchronous agent-session service, not an OpenAI chat-completions model API.</p>}{provider.id === 'devin-cli' && <p className="safe-note">Devin CLI is included in the standard GoRouter Docker image. Add your apk_user_ or cog_ key here; no separate image or CLI installation is needed for Docker. Health and model discovery authenticate without sending chat. Devin Desktop and the Devin cloud API use different keys.</p>}{provider.id !== 'devin-cli' && <Field label="Base URL"><input type="url" value={baseURL} disabled={!provider.custom_base_url && Boolean(provider.default_base_url)} onChange={(event) => setBaseURL(event.target.value)} /></Field>}<Field label={provider.id === 'devin-desktop' ? 'Devin Desktop / Windsurf key (not a Devin CLI apk_user key)' : provider.id === 'devin-cli' ? 'Devin CLI key (apk_user_ or cog_)' : provider.id === 'devin' ? 'Devin Cloud key (cog_)' : 'API key'}><input type="password" autoComplete="new-password" value={apiKey} onChange={(event) => setAPIKey(event.target.value)} /></Field><div className="dialog-actions"><button className="button" disabled={busy || !name.trim() || !apiKey.trim()} onClick={() => void submitAPIKey()}>Save encrypted connection</button></div></> : !flow ? <div className="oauth-panel"><p>{provider.description}</p><button className="button" disabled={busy || !provider.oauth_supported} onClick={() => void beginOAuth()}>Start authorization</button></div> : <div className="oauth-panel"><p>{flow.instructions}</p>{flow.user_code && <div className="device-code"><span>Device code</span><strong>{flow.user_code}</strong></div>}{(flow.verification_uri_complete || flow.verification_uri || flow.authorize_url) && <a className="button secondary" target="_blank" rel="noopener noreferrer" href={flow.verification_uri_complete || flow.verification_uri || flow.authorize_url}>Open authorization page ↗</a>}{flow.flow_type !== 'device' && <Field label="Callback URL or code"><textarea rows={4} value={callback} onChange={(event) => setCallback(event.target.value)} /></Field>}<div className="dialog-actions"><button className="button" disabled={busy} onClick={() => void finishOAuth()}>{flow.flow_type === 'device' ? 'Check authorization' : 'Complete connection'}</button></div></div>}
  </Modal>
}
