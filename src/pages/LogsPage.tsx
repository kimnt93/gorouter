...
export function LogsPage() {
  ...
  const reload = () => { setSelected(null); setVersion((value) => value + 1) }
  useEffect(() => {
    let live = true
  ...
  return <>
    <header className="page-header"><div><span className="eyebrow">Operations / Logs</span><h1>Request logs</h1><p>Inspect safe request metadata, provider routing, token accounting, cost, and latency. Prompt and completion content is never stored.</p></div><div className="page-header-actions"><span className="live-indicator"><i /> Live data</span><button className="button secondary" disabled={loading} onClick={reload}>{loading ? 'Reloading…' : 'Reload logs'}</button></div></header>
