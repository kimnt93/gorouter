...
export function AnalysisPage() {
  ...
  return <>
    ...
    {activity.loading ? <PageLoading /> : activity.error ? <PageError message={activity.error} retry={activity.retry} /> : <>
      ...
      <section className="panel"><div className="panel-header"><div><span className="eyebrow">Selected interval · each bar is 100%</span><h2>Requests by {filterState.filters.groupBy}</h2></div></div><VerticalUsageChart data={activity.data} metric="requests" /></section>
      <section className="panel"><div className="panel-header"><div><span className="eyebrow">Throughput</span><h2>Token activity</h2></div></div><VerticalUsageChart data={activity.data} metric="tokens" /></section>
