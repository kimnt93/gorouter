...
export function OrganizationModelsModal({ organization, onClose }: { organization: Organization; onClose: () => void }) {
  ...
  return <Modal title={`${organization.name} models and limits`} onClose={onClose}>
    <p>Aliases map one-to-one. Groups only bulk-assign aliases and are not callable models. Limits apply separately to each assigned user/model. Zero means unlimited. Members use their existing user key.</p>
    <ErrorBanner message={error} />
    {loading ? <p role="status">Loading organization models…</p> : <>
      <h3>Publish an alias or group</h3>
      <div className="form-grid">
        <Field label="Kind"><SearchableSelect value={kind} onChange={setKind} options={[{ value: 'alias', label: 'Alias' }, { value: 'group', label: 'Assignment package' }]} /></Field>
        <Field label="Name (without organization prefix)"><input value={name} onChange={e => setName(e.target.value)} /></Field>

      </div>
      <Field label="Source ID for alias; published alias IDs for package (one per line)"><textarea value={targets} rows={3} onChange={e => setTargets(e.target.value)} /></Field>
      <button className="button" disabled={busy || !name.trim() || !targets.trim()} onClick={() => void run(() => publishOrganizationModel(organization.id, { name, kind: kind as 'alias' | 'group', targets: targets.split('\n').map(x => x.trim()).filter(Boolean), enabled: true, weekly_limit_usd: 0 }))}>{busy ? 'Saving…' : 'Save model'}</button>
      {models.map(item => <div className="safe-note" key={item.name}><strong>{item.name}</strong><span>{item.targets.join(', ')} · {item.kind === 'group' ? 'Bulk assignment only' : 'One-to-one alias'}</span><div className="compact-actions"><button disabled={busy} onClick={() => { setName(item.name.split('/').at(-1) || ''); setKind(item.kind); setTargets(item.targets.join('\n')) }}>Edit</button><button disabled={busy} onClick={() => void run(() => publishOrganizationModel(organization.id, { ...item, name: item.name.split('/').at(-1) || '', enabled: !item.enabled }))}>{item.enabled ? 'Disable' : 'Enable'}</button></div></div>)}
      ...
      <button className="button" disabled={busy || !model || !user || invalidLimit(userLimit)} onClick={() => void run(() => assignOrganizationModel(organization.id, { model, user_id: user, enabled: true, weekly_limit_usd: userLimit === '' ? 0 : Number(userLimit) }))}>Save assignment</button>
      {grants.map(g => <div className="safe-note" key={`${g.model}:${g.user_id}`}><strong>{g.model}</strong><span>{g.user_id} · {!g.weekly_limit_usd ? 'Unlimited' : `$${g.weekly_limit_usd}/week`}</span><div className="compact-actions"><button disabled={busy} onClick={() => { setModel(g.model); setUser(g.user_id); setUserLimit(g.weekly_limit_usd === null ? '' : String(g.weekly_limit_usd)) }}>Edit limit</button><button disabled={busy} onClick={() => void run(() => assignOrganizationModel(organization.id, { ...g, enabled: !g.enabled }))}>{g.enabled ? 'Revoke' : 'Enable'}</button></div></div>)}
    </>}
  </Modal>
}
