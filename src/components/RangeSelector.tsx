import type { RangePreset, UsageFilters, User, APIKey, Organization } from '../api/contracts'
...
interface Props { filters: UsageFilters; onChange: (filters: UsageFilters) => void; users: User[]; apiKeys: APIKey[]; organizations?: Organization[]; showRange?: boolean }

export function RangeSelector({ filters, onChange, users, apiKeys, organizations = [], showRange = true }: Props) {
  const [query, setQuery] = useState('')
  const set = <K extends keyof UsageFilters>(key: K, value: UsageFilters[K]) => onChange(withAutomaticGroupBy({ ...filters, [key]: value }))
  ...
  const setType = (value: UsageFilters['filterType']) => onChange(withAutomaticGroupBy({ ...filters, filterType: value, userIds: [], apiKeyIds: [], organizationIds: [] }))
  ...
  return <div className="filter-panel usage-filter-panel">
    {showRange && <div className="filter-block"><span className="filter-label">Range</span><div className="segmented" aria-label="Date range">{ranges.map((range) => <button key={range.value} className={filters.range === range.value ? 'selected' : ''} onClick={() => set('range', range.value)}>{range.label}</button>)}</div></div>}
    ...
    {showRange && filters.range === 'custom' && <div className="custom-range"><label><span>From</span><input type="datetime-local" value={filters.since} onChange={(event) => set('since', event.target.value)} /></label><label><span>To</span><input type="datetime-local" value={filters.until} onChange={(event) => set('until', event.target.value)} /></label></div>}
  </div>
}
